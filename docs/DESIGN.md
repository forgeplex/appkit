...
| service-role 权限声明不靠手抄 SQL | `db/access.yaml` 经 `appkit db-access validate\|render\|check` 严格解析；登录角色及秘密只能外部管理，permission role 固定 NOLOGIN/NOSUPERUSER/NOBYPASSRLS/NOCREATEDB/NOCREATEROLE；标识符白名单、确定性 SQL 与拒绝覆盖已有文件把声明转成可审查的追加 migration | ▲ 生成/静态检查级：当前能证明 manifest 合法、生成物同源，**尚不能证明数据库实际 ACL、继承/PUBLIC 有效权限或 RLS 行为**；删掉旧 grant 不会自动推导 REVOKE，须在新 manifest 显式列入 forbidden/reconciliation，直到 catalog verifier 落地 |

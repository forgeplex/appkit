...
1. **schema 由调用方确定**。迁移与查询全部**不带 schema 前缀**；每次 `tx.Do`
   ...
   按令牌签发方焊入（iss=`rbac-a` ↔ 分区 `a`），内部东西向经 `X-Partition`
   头与事件 meta 传播。它与 `Meta.TenantID`（业务租户）是两个字段——分区
...
顺手把分区键焊进 `callctx`（分区域 / 分区 + 行级的域据此路由）：
...
| 多租户的数据隔离 | 生成时选形态：`-tenant`（RLS 行级）/ `-partitioned`（schema 级，§3.1）/ 两个同给（平台一套 schema + 平台内商户分行，§3.3）；建租户表照抄 0002 样例（tenant_id + 索引 + 策略）；跨租户读用 `tx.WithReadAllTenants`（只放开 SELECT） | 手写 `WHERE tenant_id = ...` 满代码（漏一条就是静默泄漏）；业务代码读 `X-Tenant-Id` / `X-Partition` 头（认证请求里已被令牌覆盖/清零）；靠 BYPASSRLS 角色或「缺 GUC = 看全部」的策略给运营开口子 |

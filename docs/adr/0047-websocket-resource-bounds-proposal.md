# ADR-0047 补充提案：WebSocket 连接与速率边界

- 状态：Proposed；尚未被接受，不改变当前 Accepted ADR-0047
...
- 提案基线：AppKit `main@ada8b630656a7390ad3a295d26ae875d98f9d5b4`；最新发布 `v0.9.6`
...
这是范围缺口，不代表已发现生产代码漏洞。此文件仅提出契约选项；在决策被接受前，不据此修改传输行为。
...
本提案暂不冻结 Go 类型名、构造器签名、正限额的最小/最大值、HTTP problem 状态码或 token bucket 内部算法；这些须在实现子 Issue 中按已有 `apperr`、超时和背压契约落定并测试。
...
- [ ] 用户/架构负责人确认方案 B，或明确选 A/C 与所需调整
- [ ] ADR 补充经独立 PR、required `ci` 与 Wen Review 接受
- [ ] 后续实现 Issue 明确 API 兼容策略、错误映射、可取消限速等待、burst 与阈值合法范围
- [ ] 测试覆盖并发 reservation 上限、pending+active 汇总、两个方向的帧/字节速率、FIFO/不丢帧、超时/取消/Host drain 和限额错误脱敏
